...
class Bot:
    ...
    def run(self) -> None:
        ...
        if cfg.provider_ids:
            self._params["providerIds"] = cfg.provider_ids
        if cfg.exclude_provider_ids:
            self._params["exceptProviderIds"] = cfg.exclude_provider_ids

        LOG.info(
            "startup service=%s country=%s maxPrice=%s providerIds=%s "
            "exceptProviderIds=%s workers=%s limit=%.1f/s",
            cfg.service,
            cfg.country,
            cfg.max_price,
